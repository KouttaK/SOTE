...
(function(global) {
  ...
  class AutocompleteManager {
    ...
    loadSettings() {
      if (typeof chrome !== 'undefined' && chrome.storage) {
        chrome.storage.sync.get(['autocompleteEnabled', 'autocompleteMinChars', 'autocompleteMaxSuggestions'], (result) => {
      ...
      if (typeof chrome !== 'undefined' && chrome.storage) {
        chrome.storage.local.get(['autocompleteUsageStats'], (result) => {
          if (result.autocompleteUsageStats) {
            this.usageStats = new Map(Object.entries(result.autocompleteUsageStats));
          }
        });
      }
    }
    ...
    findSuggestions(currentWord) {
      ...
      const matches = this.abbreviationsCache.filter(abbr => {
        ...
        return abbrLower.startsWith(lowerCurrentWord) && abbrLower !== lowerCurrentWord;
      });
      ...
      matches.sort((a, b) => {
        ...
        const aStartsExact = a.abbreviation.toLowerCase().startsWith(lowerCurrentWord);
        const bStartsExact = b.abbreviation.toLowerCase().startsWith(lowerCurrentWord);
    ...
    createSuggestionItem(suggestion, currentWord, index) {
      ...
      const formattedExpansion = this.formatExpansionForDisplay(suggestion.expansion);
    ...
    selectPrevious() {
      ...
      this.selectedIndex = this.selectedIndex <= 0 ? 
        this.currentSuggestions.length - 1 : this.selectedIndex - 1;
      this.updateSelection();
    }
    ...
    async acceptSuggestion(suggestion) {
      console.log('[Autocomplete DEBUG] acceptSuggestion chamado com:', suggestion);

      if (!this.currentElement || !suggestion) {
        console.log('[Autocomplete DEBUG] Retornando: currentElement ou suggestion é nulo/inválido.');
        return;
      }

      const { text: currentFullText, cursorPosition: currentCursorPos } = this.getCurrentText();
      console.log('[Autocomplete DEBUG] Texto atual:', currentFullText, 'Posição do cursor:', currentCursorPos);
      
      let wordStart = currentCursorPos;
      while (wordStart > 0 && !/\s/.test(currentFullText.charAt(wordStart - 1))) {
        wordStart--;
      }
      ...
      try {
        ...
        console.log('[Autocomplete DEBUG] Expansão contextualizada (após regras):', contextualExpansionText);
  ...
  function initAutocomplete() {
    ...
    document.querySelectorAll('input[type="text"], input[type="email"], input[type="search"], textarea, [contenteditable="true"]').forEach(element => {
      ...
      mutations.forEach(mutation => {
        mutation.addedNodes.forEach(node => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            if (node.matches && node.matches('input[type="text"], input[type="email"], input[type="search"], textarea, [contenteditable="true"]')) {
              autocompleteManager.attachToElement(node);
            }
            
            // Check child elements
            const editableElements = node.querySelectorAll && node.querySelectorAll('input[type="text"], input[type="email"], input[type="search"], textarea, [contenteditable="true"]');
            if (editableElements) {
