...
class StateManagerSingleton {
  constructor() {
    ...
    this.databaseHandler = SoteErrorManager.ErrorHandlerFactory.createDatabaseHandler();
    this.networkHandler = SoteErrorManager.ErrorHandlerFactory.createNetworkHandler();
  ...
  async getInstance() {
    ...
    if (this.isInitializing) {
      ...
      throw new Error("A inicialização concorrente falhou em resolver uma instância válida.");
    }
  ...
  async _initializeWithErrorHandling() {
    const transactionId = `init_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    return await this.degradationManager.executeWithFallback(
      ...
      async (context) => {
        ...
        SoteErrorManager.Logger.warn('Inicializando com fallback - dados mínimos', context);
  ...
  async _initialize(context) {
    try {
      SoteErrorManager.Logger.info('Iniciando inicialização completa do StateManager', context);
      ...
      SoteErrorManager.Logger.info("StateManager inicializado com sucesso", context);
      return stateManager;
  ...
  async _initializeMinimal(context) {
    try {
      SoteErrorManager.Logger.warn('Inicializando StateManager em modo mínimo', context);
      ...
      SoteErrorManager.Logger.info("StateManager inicializado em modo mínimo", context);
      return stateManager;
    ...
    return await this.degradationManager.executeWithFallback(
      ...
      async (dbContext) => {
        ...
        await new Promise((resolve, reject) => {
          const transaction = db.transaction(SOTE_CONSTANTS.STORE_ABBREVIATIONS, 'readonly');
          const store = transaction.objectStore(SOTE_CONSTANTS.STORE_ABBREVIATIONS);
          const request = store.count();
          
          request.onsuccess = () => resolve(request.result);
          request.onerror = () => reject(SoteErrorManager.createError(
            'Falha no teste de leitura do banco',
            SoteErrorManager.ERROR_CODES.TEMPORARY.DATABASE_TIMEOUT,
            'TEMPORARY',
            dbContext
          ));
          transaction.onerror = () => reject(SoteErrorManager.createError(
        ...
        SoteErrorManager.Logger.info("Verificação de saúde do banco concluída", dbContext);
        return true;
      },
  ...
  _startHealthCheck() {
    ...
    this.healthCheckInterval = setInterval(async () => {
      ...
      try {
        await this._performDatabaseHealthCheck({ transactionId, operation: 'health_check' });
        SoteErrorManager.Logger.debug('Verificação de saúde periódica bem-sucedida', { transactionId });
      } catch (error) {
        SoteErrorManager.Logger.error("Falha na verificação de saúde periódica", { 
          transactionId, 
  ...
  async _getCachedAbbreviations(context) {
    ...
    SoteErrorManager.Logger.debug("Abreviações carregadas do banco e armazenadas no cache", context);
...
async function refreshStateFromDB() {
  ...
  try {
    ...
    const abbreviationsResult = await stateManagerSingleton.degradationManager.executeWithFallback(
      'database',
      async (context) => {
      ...
      async (context) => {
        ...
        SoteErrorManager.Logger.warn('Usando fallback para abreviações', context);
        return stateManagerSingleton._getCache('abbreviations') || [];
      },
      { transactionId, operation: 'refresh_state' }
    );
    
    await stateManager.setState({ abbreviations: abbreviationsResult.data || [] }, "REFRESH_FROM_DB");
...
async function initializeApp() {
  const transactionId = `app_init_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  
  try {
    SoteErrorManager.Logger.info("Inicializando aplicação", { transactionId });
    await stateManagerSingleton.getInstance();
    SoteErrorManager.Logger.info("Aplicação inicializada com sucesso", { transactionId });
  } catch (error) {
    const soteError = SoteErrorManager.createError(
...
function handleMessage(message, sender, sendResponse) {
  (async () => {
    const transactionId = `msg_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    try {
      ...
      switch (type) {
        ...
        case MESSAGE_TYPES.ADD_ABBREVIATION:
          if (!payload) throw SoteErrorManager.createError(
          ...
          await stateManagerSingleton.databaseHandler.execute(async (context) => {
        ...
        case MESSAGE_TYPES.IMPORT_ABBREVIATIONS:
          if (!payload?.data) throw SoteErrorManager.createError(
            "Dados obrigatórios para IMPORT_ABBREVIATIONS",
            SoteErrorManager.ERROR_CODES.PERMANENT.MISSING_REQUIRED_FIELD,
            'PERMANENT',
            { transactionId, type }
          );
          ...
          }, { transactionId, operation: 'import_abbreviations' });
        ...
        case MESSAGE_TYPES.DELETE_RULE:
          if (!payload?.ruleId) throw SoteErrorManager.createError(
        ...
        case MESSAGE_TYPES.ADD_CHOICE:
          if (!payload?.options) throw SoteErrorManager.createError(
          ...
          const newChoiceId = await stateManagerSingleton.databaseHandler.execute(async (context) => {
        ...
        case MESSAGE_TYPES.GET_CHOICE_CONFIG:
          if (!payload?.id) throw SoteErrorManager.createError(
          ...
          const choiceData = await stateManagerSingleton.databaseHandler.execute(async (context) => {
        ...
        default:
          SoteErrorManager.Logger.warn(`Tipo de mensagem desconhecido recebido: ${type}`, { 
            transactionId, 
            type 
          });
      ...
      const soteError = error instanceof SoteErrorManager.SoteError ? error : 
      ...
      logError(`Erro ao processar a mensagem ${message?.type || 'UNKNOWN'}`, soteError);
...
chrome.storage.onChanged.addListener(async (changes, namespace) => {
  ...
  const transactionId = `storage_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  try {
    ...
    if (settingsChanged) {
      ...
      await stateManager.setState({ settings: newSettings }, "SETTINGS_SYNC_UPDATE");
    }
  } catch (error) {
    const soteError = SoteErrorManager.createError(
...
self.addEventListener("beforeunload", () => {
  SoteErrorManager.Logger.info("Service Worker sendo descarregado - limpando recursos");
...
setInterval(() => {
  if (!stateManagerSingleton.isInitializing && stateManagerSingleton.instance) {
    const componentStatus = stateManagerSingleton.degradationManager.getComponentStatus();
    const circuitBreakerState = SoteErrorManager.globalCircuitBreaker.getState();
